...
<div class="page-container">
	<div class="text-l"> 收件人：
		<input type="text" class="input-text" style="width:150px" placeholder="输入姓名" id="recvName" maxlength="20">
		&nbsp;&nbsp;
		手机号:
		<input type="text" class="input-text" style="width:150px" placeholder="输入手机号" id="recvPhone" maxlength="11">
...
function createPage(){
	...
	if($.trim($("#recvName").val()) != ""){
		reqParam['recvName'] = $.trim($("#recvName").val());
	}
	if($.trim($("#recvPhone").val()) != ""){
		reqParam['recvPhone'] = $.trim($("#recvPhone").val());
	}
	$.ajax({
		type : "get",
		timeout : 10000,
		...
		success : function(data) {
			...
			for(var i = 0; i< obj.length;i++){
				...
				html += "<td  class=\"text-c\">"+obj[i].recvName+"</td>";
				html += "<td  class=\"text-c\">"+obj[i].recvPhone+"</td>";
				html += "<td  class=\"text-c\">"+(obj[i].province.provinceName+obj[i].city.cityName+obj[i].district.districtName+obj[i].recvAddr)+"</td>";
				html += "<td  class=\"text-c\">"+obj[i].createDate+"</td>";
				html += "<td  class=\"text-c\">"+(obj[i].status==0?"冻结":"活跃")+"</td>";
